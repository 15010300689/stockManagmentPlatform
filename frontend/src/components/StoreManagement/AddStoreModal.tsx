...
interface AddStoreModalProps {
    ...
    isOpen: boolean;
    onCancel: () => void;
    onSubmit?: (values: Partial<StoreItem>, mode: 'add' | 'edit') => void;
    currentStoreInfo?: Partial<StoreItem>;
}

function AddStoreModal(props: AddStoreModalProps): JSX.Element {
    ...
    useEffect(() => {
        if (mode === 'edit' && currentStoreInfo.id) {
            form.setFieldsValue({
                ...
                status: currentStoreInfo.status === '1'
            } as StoreFormValues);
        } else {
            form.resetFields();
            form.setFieldsValue({
    ...
    }, [mode, isOpen, currentStoreInfo, form]);
